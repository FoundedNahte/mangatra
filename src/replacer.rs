...
// Convert to an blank ImageBuffer from the image crate to allow text writing in different fonts
...
pub fn write_text(
    ...
    */
    for i in 0..translated_text.len() {
        let region = text_regions.get(i)?;
        let text = translated_text[i].clone();

        let mut canvas = mat_to_image_buffer(region)?;
    ...
    for canvas in canvases {
        cv_vector.push(image_buffer_to_mat(canvas)?);
    }
...
/*
    This function essentially stitches back the translated text regions into the original image.
    The original image is sliced into four difference pieces around the text region, and they are glued back together.
*/
pub fn replace_text_regions(
    ...
    let mut temp_image = core::Mat::copy(original_image)?;

    for i in 0..text_regions.len() {
        let (text_region_x, text_region_y) = origins[i];
        ...
        let (bottom_panel_x, bottom_panel_y) = (text_region_x, text_region_y + text_region_height);
