...
use opencv::{
    self as cv, core::Rect2i,  core::ToInputArray, dnn, highgui, imgcodecs, prelude::*,
};
use std::cmp::max;
...
fn run_inference() -> Result<()> {
    ...
    let class_ids = detections.class_ids;
    let confidences = detections.confidences;
    let boxes = detections.boxes;

    let mut original_image: cv::core::Mat = imgcodecs::imread("img.jpg", imgcodecs::IMREAD_COLOR)?;
